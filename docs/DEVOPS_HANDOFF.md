...
| `GOOGLE_AI_API_KEY` | **Yes** | Gemini API for AI edge functions (`parse-resume`, `enrich-profile`, `analyze-candidate`, `salary-market-check`, etc.) | `AIza...` |
| `GEMINI_API_KEY` | No | Alias for `GOOGLE_AI_API_KEY` | — |
| `TAVILY_API_KEY` | For Market check | Tavily Search API on **functions** container only (`salary-market-check`). Never send to the browser. | `tvly-...` |
...
**SAML 2.0 SSO (self-hosted GoTrue only — Slice B):**

| Variable | Required | Purpose | Example |
|----------|----------|---------|---------|
| `GOTRUE_SAML_ENABLED` | For SSO | Enable SAML SSO endpoints on auth service | `true` |
| `GOTRUE_SAML_PRIVATE_KEY` | For SSO | Base64-encoded RSA private key for SP signing | `<base64 PEM>` |
| `API_EXTERNAL_URL` | For SSO | Public URL of Supabase API (metadata + ACS must be reachable) | `https://spxtalent-db.clientwork.xyz` |

Expose Kong routes (no JWT): `/auth/v1/sso/saml/metadata` and `/auth/v1/sso/saml/acs`. Register each IdP at runtime via `POST /auth/v1/admin/sso/providers` (service role) — no restart per IdP. Client uses `supabase.auth.signInWithSSO({ domain })`. Enable the login button in Settings → Security after IdP registration.
