...
| Company website lookup | `enrich-company-websites` | `supabase/functions/enrich-company-websites/index.ts` | system + user | No | Tavily search → Gemini picks the official site from the hits |
...
- **Injected:** Full candidate profile; job title for fit sentence; `red_flag_rules` numeric thresholds; **COMPUTED TENURE** from `_shared/candidateTenure.ts` (relevant vs professional years, gaps, trainings). Summary must not invent calendar-span years.
- **Output:** `enrich_profile` tool → updates `candidates` (skills_tags, ai_summary, red_flags, experience_years from computed professional total).

### salary-market-check

- **Purpose:** Recruiter-triggered INR LPA range from Tavily snippets + Gemini extraction.
- **Injected:** Relevant years from `candidateTenure` (not calendar span), job title/JD, location.
- **Output:** min/max LPA + 2–3 source URLs, or `insufficient_sources`. Cached on `external_refs`.
- **Env:** `TAVILY_API_KEY` (functions host only) + Gemini key.
...
- **Purpose:** Resolve work-experience company names → official homepage URLs.
- **Key symbols:** `searchCompany()` (Tavily, one query per company, concurrency 3); `pickOfficialSites()` (single Gemini call for the whole batch).
- **Injected:** Cleaned company names plus their Tavily hits (URL, title, snippet). No candidate contact details or résumé text.
- **Output:** `pick_official_sites` tool → `{ name, found, website }[]`. A pick is discarded unless its host appeared in that company's Tavily hits, so a hallucinated domain can never be stored.
- **Rejected sources:** LinkedIn, Facebook, Naukri, AmbitionBox, Glassdoor, Wikipedia, job boards and data-broker/directory domains are filtered out of the hits before Gemini sees them (and never fetched).
- **Cache:** `external_refs` (`provider = tavily_company`, `external_id = company|<normalized name>|v1`, 30-day TTL), including negative results, so repeat drawer opens cost one `select`.
