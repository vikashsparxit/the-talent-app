...
> **Status:** Slice B **partial** shipped on prod `2c55a37` (migration applied) — see **Built in Slice B** below. Full phased plan remains for Phase 2–4. **Built vs leftover tracker:** `ROADMAP.md` Enterprise credibility table.
...
## Slice B — Built vs still missing (honest)

### ✅ Shipped in Slice B (code + migration SQL for review)

| Item | What shipped |
|------|----------------|
| **Append-only audit log** | `audit_log` table + Settings → Security viewer; writes on staff candidate/role/interview actions |
| **Staff TOTP MFA (app)** | Settings → Security enrolment + login MFA challenge (`useAuth.tsx`). **Requires** self-hosted GoTrue TOTP flags (ops) |
| **SAML SSO (app stub)** | `signInWithSSO()`, Settings → Security IdP UI, IT runbook in `docs/DEVOPS_HANDOFF.md`. **Requires** GoTrue SAML env + IdP registration (ops) |
| **Sub-processor disclosure** | Settings → Compliance lists Supabase, AWS SES, Google Gemini, Piston; documented in this file |
| **Grievance officer** | `system_config.compliance_settings` — name/email in Settings; shown on applicant footer |
| **Privacy / terms URLs** | Settings → Compliance; linked from applicant profile consent and footer |
| **Sensitive-field consent** | Checkbox + `sensitive_data_consent_at` / `sensitive_data_consent_source` on `applicant_profiles` when saving DOB, gender, marital status, blood group |
| **No fake certifications** | No SOC 2 / ISO 27001 claims in product or docs |

### ❌ Still missing (deferred — do not claim in RFPs)

| Item | Target |
|------|--------|
| Consent at careers apply, applicant signup, bulk import | Phase 1 remainder |
| `data_deletion_requests` + HR erasure queue | Phase 3 |
| Retention automation cron | Phase 4 |
| Full `notification_prefs` / marketing enforcement audit | Phase 2 |
| Cookie notice + preference UI | Phase 1/2 |
| Breach notification runbook (product table) | Phase 1 doc / Phase 4 optional table |
| Applicant self-service DSAR export | Phase 3 |
| Admin MFA reset (staff account recovery) | Phase 1 remainder |

**We do not claim full GDPR/DPDP compliance until Phase 1–3 are complete.** Slice B provides honest hooks for enterprise questionnaires — not a compliance certificate.
...
| ROADMAP item | Gap |
|--------------|-----|
| **Lawful basis & consent at every collection point** | Exam portal + **applicant profile sensitive fields** (DOB, blood group, gender, marital status) have consent. **Still missing:** careers apply (`Careers.tsx`, `QuickApplyDialog`), applicant signup (`ApplicantLogin.tsx`), bulk import (`BulkImportDialog.tsx`), manual candidate add (`Candidates.tsx`). No `consent_*` on `candidates`. No `lawful_basis` metadata. No withdrawal flow. |
...
| **Privacy policy hooks** | **Partial (Slice B).** Settings → Compliance: privacy/terms URLs, grievance officer; applicant footer links. Staff `Auth.tsx` still has generic "terms of service" text with no tenant policy link. |
...
| **DPA / sub-processors** | **Partial (Slice B).** In-product list in Settings → Compliance. Processors: Supabase, AWS SES, Google Gemini, Piston (self-host recommended). |
...
| **India DPDP specifics** | **Partial (Slice B).** Grievance officer in Settings + footer; sensitive-field consent on applicant profile. Consent manager, children's-data exclusion, localization guidance still missing. |
| **OSS documentation** | **`docs/COMPLIANCE.md` exists** with built vs missing (Slice B). |
...
`applicant_profiles` collects **DOB** (`dob_documented` required in profile modal), **blood group**, **gender**, **marital status**, **emergency phone** — **sensitive fields require explicit consent on save** (Slice B); careers apply, signup, and bulk import still lack consent. Digital application form and pre-screen add more PII. Resume/assessment data is sent to **Gemini** edge functions.
...
|--------------|---------|--------------|
| Lawful basis & consent (all collection points) | Exam + applicant profile sensitive fields | P1 + P2 |
| Data subject rights | None | P3 |
| Privacy policy hooks + Compliance tab | **Partial (Slice B)** | P1 remainder |
| `notification_prefs` enforcement | Partial | P2 |
| Data retention | None | P4 |
| Breach process | None | P1 (doc), P4 (optional table) |
| DPA / sub-processors | **Partial (Slice B)** | P1 |
| Cookie / tracking | None | P1 (inventory + notice) |
| India DPDP (grievance, consent manager, localization) | **Partial (Slice B)** | P1 + P2 + docs |
| OSS `COMPLIANCE.md` | ✅ | Maintain |
...
*Last updated: 2 Sep 2026 — Slice B partial ship on prod `2c55a37`. Migration `20260902000001_audit_log_compliance.sql` applied.*
