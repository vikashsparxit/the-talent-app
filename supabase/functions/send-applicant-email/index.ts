...
function jobTitleFromRelation(job: unknown, fallback?: string): string {
  const row = Array.isArray(job) ? job[0] : job;
  const title = (row as { title?: string } | null)?.title?.trim();
  return title || fallback || "the position";
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { type, rejection_reason, application_id, candidate_id } = body;
    ...
    const loadApplication = async (id: string) => {
      const { data } = await supabaseAdmin
        ...
        .select("id, applicant_email, applicant_name, created_at, job:jobs(id, title)")
        .eq("id", id)
        .maybeSingle();
      return data;
    };

    const recipientFromApplication = (app: {
      applicant_email: string;
      applicant_name: string;
      job?: unknown;
    }): ResolvedRecipient => ({
      ...
      jobTitle: jobTitleFromRelation(app.job, body.job_title),
    });
    ...
    if (type === "application_received") {
      ...
      if (application_id) {
        const appRow = await loadApplication(application_id);
        if (appRow?.id && appRow.created_at >= since) {
          resolved = recipientFromApplication(appRow);
        }
      }

      if (!resolved && body.applicant_email && body.job_title) {
        const { data: jobRow } = await supabaseAdmin
          .from("jobs")
          .select("id")
          .ilike("title", body.job_title)
          .limit(1)
          .maybeSingle();
        ...
        if (jobRow?.id) {
          const { data: recentApp } = await supabaseAdmin
            .from("job_applications")
            .select("id, applicant_email, applicant_name, job:jobs(id, title)")
          ...
          if (recentApp?.id) {
            resolved = recipientFromApplication(recentApp);
          }
        }
      }
      ...
      applicant_email = resolved.email;
      applicant_name = resolved.name;
      ...
      if (application_id) {
        const appRow = await loadApplication(application_id);
        if (appRow?.id) resolved = recipientFromApplication(appRow);
      }

      if (!resolved && candidate_id) {
        const { data: cand } = await supabaseAdmin
          .from("candidates")
          .select("id, name, email, job:jobs(id, title)")
          .eq("id", candidate_id)
          .maybeSingle();
        if (cand?.email) {
          resolved = {
            email: cand.email,
            name: cand.name || body.applicant_name || "Applicant",
            jobTitle: jobTitleFromRelation(cand.job, body.job_title),
          };
        }
      }

      if (!resolved && body.applicant_email) {
        const { data: cand } = await supabaseAdmin
          .from("candidates")
          .select("id, name, email, job:jobs(id, title)")
          .eq("email", body.applicant_email)
          .limit(1)
          .maybeSingle();
        if (cand?.email) {
          resolved = {
      ...
      if (!resolved) {
        return new Response(JSON.stringify({ error: "Could not resolve recipient" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
