...
serve(async (req) => {
  ...
  try {
    ...
    const userMessage = [
      ...
      "Days open (since job created): " + (metrics.days_open != null ? metrics.days_open + " days" : "N/A"),
      "Scenario hint: " + (metrics.scenario || "unknown") +
        (metrics.limited_sample ? " (limited sample: N < 10)" : ""),
      "",
      "PIPELINE METRICS:",
      "- Total candidates (N): " + metrics.total_candidates,
      ...
      "- Hired: " + (metrics.hired_count != null ? metrics.hired_count : 0) + " of " + metrics.total_candidates,
      "- Conversion rate (% of pipeline candidates hired): " + (metrics.conversion_rate != null ? metrics.conversion_rate + "%" : "N/A"),
    ...
    const systemPrompt = [
      ...
      "- conversion_rate = % of pipeline candidates who are hired (hired_at set, or legacy shortlisted status). Not last-stage reach.",
      "",
      "LOW-N / SAMPLE-SIZE RULES (mandatory):",
      "- Let N = total_candidates. limited_sample means N < 10.",
      "- When N < 10: NEVER describe rates as high/low/poor/critical. Prefer counts (\"1 of 6 no-shows\", \"0 of 6 hired\").",
      "- Prefer phrases: \"early signal\", \"limited sample\", \"too early to judge rates\".",
      "- No-show may be called \"high\" ONLY if (no_show/N >= 20% AND N >= 10) OR no_show >= 3 events. Otherwise state the count only.",
      "- Scenario guidance (use metrics.scenario when present):",
      "  * early_thin (low N + days_open < ~14): DO NOT panic about conversion. Recommendation = quick-closure playbook: clear Pending Approval → schedule first interviews → then source more. Match Action Queue language (Approve pending / Schedule / Decide).",
      "  * aged_thin (low N + aged/quiet/deadline pressure): primary concern is thin funnel / sourcing volume — not rate theatre.",
      "  * adequate (N >= 10): rates and percentage comparisons are allowed.",
      "",
      ...
      "- When N < 10: SKIP rate-based penalties and bonuses for conversion_rate, no_show %, and hold % (do not apply those ± adjustments). Keep feedback_coverage, interview quality, and process signals.",
      "- When N >= 10:",
      "  * conversion_rate >= 20%: +15; 10-19%: +5; 5-9%: no change; < 5%: -15",
      "  * no_show / total_candidates > 20%: -15; 10-20%: -5",
      "  * hold / total_candidates > 30%: -10; 15-30%: -5",
      "- feedback_coverage_pct >= 80%: +15; 60-79%: +5; 40-59%: no change; < 40%: -15 (always apply when scheduled interviews exist)",
      ...
      "GRADE LABEL: Excellent / Good / Fair / Poor / Critical — when N < 10 prefer milder labels (Fair/Good) unless stall/deadline/process failure is clear; avoid Critical from rates alone.",
      "",
      "INSIGHTS: 2-4 factual observations directly from the data. Use counts when N < 10. Never fabricate.",
      "RISKS: Only genuine concerns. Empty array if pipeline is healthy or merely early/thin without process stalls. Do not invent conversion/no-show crises from tiny N.",
      "RECOMMENDATION: One short actionable step matching the scenario. Prefer Action Queue verbs: Approve pending, Schedule interviews, Decide (advance/hire), Chase overdue feedback, Source more candidates. Max ~140 characters. No essay.",
    ].join("\n");
