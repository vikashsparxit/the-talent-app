...
import { downloadResumeBytes, extractResumeStoragePath, isSafeResumeStoragePath, userCanParseResume } from "../_shared/resume.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!isSafeResumeStoragePath(path)) {
      return new Response(JSON.stringify({ error: "Invalid resume path" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!auth.isServiceRole) {
      ...
      const allowed = await userCanParseResume(supabase, auth.userId!, email, resume_url);
      if (!allowed) {
    ...
    const aiResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${GOOGLE_AI_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          systemInstruction: {
            parts: [{
              text: "You are a resume parser. Extract structured information from the resume provided. Be accurate and extract only what is clearly stated in the document. If a field is not present, omit it entirely — do NOT return 'None', 'N/A', 'null', or placeholder strings. Return only real values. experience_years must NOT be calendar span from first job to last, and must NOT count internships/trainees/student roles or short courses as professional years. Prefer omitting experience_years if unsure — a deterministic post-pass will compute it.",
            }],
          },
          contents: [{
            role: "user",
            ...
            functionDeclarations: [{
              ...
              parameters: {
                ...
                properties: {
                  ...
                  experience_years: { type: "number", description: "Professional employed years only: exclude internships/trainees/student roles and short courses. Do not use first-to-last calendar span. Omit if unsure." },
    ...
    const career = analyzeCareer({
      ...
      target: { jobTitle: parsed.current_role ?? null, roleApplied: parsed.current_role ?? null },
    });
    if (career.roles.length > 0) {
      parsed.experience_years = career.totalProfessionalYears;
    }
