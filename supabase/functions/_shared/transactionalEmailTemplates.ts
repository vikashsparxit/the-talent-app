...
export function buildCandidateInterviewScheduledEmail(
  branding: EmailBranding,
  params: {
    recipientName: string;
    jobTitle: string;
  ...
  const { companyName, primaryColor } = branding;
  const { recipientName, jobTitle, stageName, scheduledDisplay, modeLabel, meetingLink } = params;
  ...
  const bodyHtml = [
    ...
    emailParagraph(`Hi ${escapeHtml(recipientName)},`),
    emailParagraph(
      `Your interview for <strong>${escapeHtml(jobTitle)}</strong> has been scheduled.`,
    ),
    emailInfoBox(
      `<p style="margin:0 0 8px 0;color:#52525b;font-size:15px;"><strong>Stage:</strong> ${escapeHtml(stageName)}</p>
       <p style="margin:0 0 8px 0;color:#52525b;font-size:15px;"><strong>When:</strong> ${escapeHtml(scheduledDisplay)}</p>
    ...
    emailParagraph("A calendar invite is attached — add it to your calendar in one click."),
    emailSignOff(companyName),
  ].join("");
...
const CANCEL_REASON_LABELS: Record<string, string> = {
  ...
  candidate_emergency: "Candidate emergency",
  candidate_withdrew_slot: "Candidate withdrew from this slot",
  other: "Other",
};
...
export function buildInterviewCancelledEmail(
  ...
  params: {
    ...
    jobTitle: string;
    stageName: string;
    previousScheduledDisplay: string;
    reasonLabel: string;
    note?: string | null;
    candidateProfileUrl?: string | null;
  },
  ...
  const noteBlock = note
    ? `<p style="margin:12px 0 0 0;color:#52525b;font-size:15px;line-height:1.6;"><strong>Note:</strong> ${escapeHtml(note)}</p>`
    : "";
  ...
  const bodyHtml = [
    ...
    emailParagraph(`Hi ${escapeHtml(recipientName)},`),
    emailParagraph(
      `The interview with ${linkedCandidateName(candidateName, candidateProfileUrl, primaryColor)} for <strong>${escapeHtml(jobTitle)}</strong> has been cancelled. The candidate remains in the same pipeline stage.`,
    ),
    emailInfoBox(
      `<p style="margin:0 0 8px 0;color:#52525b;font-size:15px;"><strong>Stage:</strong> ${escapeHtml(stageName)}</p>
       <p style="margin:0 0 8px 0;color:#52525b;font-size:15px;"><strong>Was scheduled:</strong> ${escapeHtml(previousScheduledDisplay)}</p>
       <p style="margin:0;color:#52525b;font-size:15px;"><strong>Reason:</strong> ${escapeHtml(reasonLabel)}</p>
       ${noteBlock}`,
      primaryColor,
    ),
    profileCta,
    emailParagraph(`Check the pipeline in ${escapeHtml(companyName)} for the next step.`),
    emailSignOff(companyName),
  ].join("");
  ...
  return {
    ...
    text: [
      `Hi ${recipientName},`,
      "",
      `The interview with ${candidateName} for ${jobTitle} has been cancelled.`,
      "",
      `Stage: ${stageName}`,
      `Was scheduled: ${previousScheduledDisplay}`,
      `Reason: ${reasonLabel}`,
      note ? `Note: ${note}` : "",
      candidateProfileUrl ? `\nView candidate profile: ${candidateProfileUrl}` : "",
      "",
      `Check the pipeline in ${companyName} for the next step.`,
      "",
      `— ${companyName} Hiring Team`,
    ].filter(Boolean).join("\n"),
  };
