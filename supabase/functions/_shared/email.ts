...
async function sendViaSmtp(options: {
  ...
  const info = await transporter.sendMail({
    ...
    ...(calendar
      ? {
          icalEvent: {
            filename: calendar.filename ?? "invite.ics",
...
export function organizerEmailFromFromAddress(from: string): string {
  const angled = from.match(/<([^>]+)>/);
  if (angled?.[1]?.includes("@")) return angled[1].trim();
  const trimmed = from.trim();
  return trimmed.includes("@") ? trimmed : "noreply@thetalentapp.io";
}
