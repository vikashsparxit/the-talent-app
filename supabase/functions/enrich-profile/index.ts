  ...
  try {
    ...
    const career = analyzeCareer({
      ...
      target: {
        ...
        requiredSkills: Array.isArray(candidate.job?.required_skills) ? candidate.job.required_skills : null,
      },
      gapThresholdMonths: employmentGapMonths,
    });
...
Stored Experience Years (may be wrong — ignore if it contradicts COMPUTED TENURE): ${candidate.experience_years || "N/A"}
...
TENURE RULES (mandatory — COMPUTED TENURE in the user message is ground truth):
- Never say "X years of experience" unless X is the RELEVANT professional figure from COMPUTED TENURE.
- Do not use calendar span, stored Experience Years, or the sum of every résumé row.
- Internships/trainees/student/short courses (< ~6 months, titles like Trainee/Intern/Student) are training, not professional years — even if they sit during a degree.
- Down-weight or label roles that do not match the target job (e.g. university Project Coordinator vs Systems Architect / cloud).
- If dates overlap, do not double-count.
- Call out material employment gaps honestly.
- Prefer phrasing like: "About ${career.relevantYears} years of relevant experience (${career.relevantRoleLabels.join(", ") || "see COMPUTED TENURE"}). Earlier unrelated roles and short trainings are not counted as equivalent seniority."
1. Who they are (recent role + company + relevant years only)
...
4. Any notable flag: certs, career gaps, education-track mismatch, or incomplete profile

RED FLAGS — detect these patterns (only flag when clearly evident):
- employment_gap: use the COMPUTED TENURE gap list (same ≥ ${employmentGapMonths} month threshold). Do not invent different dates.
...
Max 15 skills. Summary max 80 words.`,
              ...
              function: {
                ...
                parameters: {
                  ...
                  properties: {
                    ...
                    experience_years: {
                      type: "number",
                      description: "Must equal COMPUTED TENURE total professional years (trainings excluded, overlaps merged). Never calendar span.",
                    },
                    suggested_role: {
                      type: "string",
    ...
    if (!aiSummary) {
      ...
      try {
        const summaryPrompt = `Write a 3-4 sentence internal recruiter brief. Be factual and direct. Max 80 words. No headers.
Never claim years of experience except the relevant professional figure from COMPUTED TENURE. Do not count trainings/interns or unrelated roles as seniority. Call out material gaps.\n\n${profileSummary}`;
    ...
    const withoutAiGaps = rawRedFlags.filter((f: { type?: string }) => f?.type !== "employment_gap");
    const computedGapFlags = career.gaps.map((g) => ({
      type: "employment_gap",
      message: g.message,
      severity: g.months >= 12 ? "medium" : "low",
    }));
    const redFlags = filterStaleLinkedInRedFlags(
      [...withoutAiGaps, ...computedGapFlags],
    ...
    if (career.roles.length > 0) updateData.experience_years = career.totalProfessionalYears;
    else if (enrichment.experience_years != null && !candidate.experience_years) updateData.experience_years = enrichment.experience_years;
