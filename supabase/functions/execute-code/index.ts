...
/**
 * Piston API root (POST {url}/execute).
 *
 * emkc.org is a community-run, unauthenticated, no-SLA sandbox. Sending
 * candidate-submitted code there is an enterprise security finding — not a
 * feature. Production must set PISTON_URL to a self-hosted instance.
 * Local Supabase (localhost / 127.0.0.1) may fall back so eval still works.
 */
function resolvePistonApi(): string {
  const configured = Deno.env.get('PISTON_URL')?.trim();
  ...
  throw new Error(
    'PISTON_URL is not set. Self-host Piston and set PISTON_URL (do not use emkc.org in production).',
  );
}
...
async function executeCode(
  ...
  try {
    ...
    try {
      response = await fetch(`${pistonApi}/execute`, {
        method: 'POST',
...
serve(async (req) => {
  ...
  try {
    ...
    const pistonApi = resolvePistonApi();

    if (!test_cases || test_cases.length === 0) {
      // If no test cases, just run the code
      const result = await executeCode(pistonApi, code, language, '');
      return new Response(
        JSON.stringify({
    ...
    for (const testCase of test_cases) {
      const result = await executeCode(pistonApi, code, language, testCase.input);
