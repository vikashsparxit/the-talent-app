...
**Edge function:** `score-pipeline` | **UI:** Reports → By Job → "Analyse Pipeline"

When a recruiter or HR manager wants a quick read on a job's hiring pipeline, they click "Analyse Pipeline" on the Reports page. The frontend pre-computes a rich metrics payload (stage funnel counts, conversion rate, feedback coverage %, avg days between stages, days since last activity, days to deadline, avg interview scores) and sends it to Gemini along with deterministic scoring rules baked into the system prompt.
...
- One `recommendation` — the single most impactful action

The result is displayed as a score card with a colour-coded grade badge (emerald → red) and progress bars. It is **ephemeral** — never written to the database. Switching jobs or refreshing triggers a new Gemini call.
...
| Pipeline Health Scorer | On demand (button, Reports page) | Yes | No |
