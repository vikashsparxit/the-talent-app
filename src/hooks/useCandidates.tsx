...
import { lookupApplicantAvatarUrls } from '@/lib/candidateDetail';
...
function candidateFieldOrPattern(pattern: string): string {
  return CANDIDATE_TEXT_SEARCH_FIELDS.replaceAll('{pattern}', pattern);
}
...
function applyCandidateTextSearch<T extends { or: (filters: string) => T }>(query: T, search: string): T {
  ...
  // Boolean queries keep legacy whole-string matching; client-side parser handles them.
  if (/\b(AND|OR|NOT)\b/i.test(term)) {
    const pattern = `%${escapeIlike(term)}%`;
    return query.or(candidateFieldOrPattern(pattern));
  }
  ...
  for (const token of tokens) {
    ...
    result = result.or(candidateFieldOrPattern(pattern));
  }
  return result;
}
...
async function mapCandidatesWithOwners(data: any[]): Promise<Candidate[]> {
  ...
  const avatarMap = await lookupApplicantAvatarUrls((data || []).map((c: { email?: string | null }) => c.email));
  ...
  return (data || []).map((c: any) => ({
    ...
    avatar_url: c.email ? (avatarMap.get(normalizeApplicantEmail(c.email)) ?? null) : null,
  })) as Candidate[];
}
  ...
  const updateCandidate = useMutation({
    mutationFn: async ({ id, skills, job_id, ...updates }: { 
      ...
      const { data: beforeRow } = await supabase
        .from('candidates')
        .select('name, email, phone, role_applied, job_id, candidate_status, notes')
        .eq('id', id)
        .maybeSingle();
      ...
      void writeAuditLog({
        ...
        entityId: id,
        before: beforeRow ? (beforeRow as Record<string, unknown>) : null,
        after: updateData,
      });
