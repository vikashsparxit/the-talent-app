...
export function useJobs(options?: { summary?: boolean; reportList?: boolean }) {
    ...
    queryFn: async () => {
      if (summary) {
        const { data, error } = await supabase
          .from('jobs')
          .select('id, title, status, created_at, application_deadline, total_openings, positions_filled, sort_order, status_note')
      ...
      if (reportList) {
        const { data, error } = await supabase
          .from('jobs')
          .select('id, title, status, application_deadline, created_at, department, domain, sort_order, status_note, candidates(count)')
          .order('sort_order', { ascending: true })
          .order('created_at', { ascending: false });
  ...
  const createJob = useMutation({
    mutationFn: async (data: CreateJobData) => {
      const topSort = jobs.reduce((min, job) => Math.min(min, job.sort_order ?? 0), 0);
  ...
  const reorderJobs = useMutation({
    ...
    onMutate: async (updates) => {
      ...
      queryClient.setQueriesData({ queryKey: ['jobs'] }, (old) => {
        ...
        return [...old]
          .map((job: Job) => {
            const nextOrder = orderMap.get(job.id);
            return nextOrder === undefined ? job : { ...job, sort_order: nextOrder };
          })
          .sort((a: Job, b: Job) => (a.sort_order ?? 0) - (b.sort_order ?? 0) || b.created_at.localeCompare(a.created_at));
      });
      return { previous };
    },
    onError: (error: Error, _updates, context) => {
      ...
      toast({ title: 'Failed to reorder jobs', description: error.message, variant: 'destructive' });
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
    },
