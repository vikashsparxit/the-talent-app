...
/**
 * Pipeline badge mapping (distinct from Evaluations list labels):
 * - invited / in_progress → pending (not finished by candidate)
 * - completed → awaiting_review when not auto-passed; passed when score already clears threshold
 *   (Evaluations still shows "Pending Review" until Mark as Evaluated)
 * - evaluated → passed / failed from the `passed` column (HR has signed off)
 */
export function resolvePipelineAssessmentStatus(
  ...
  if (row.status === 'completed') {
    // Auto-pass still unlocks advance; do not scream "failed" before HR review.
    return row.passed === true ? 'passed' : 'awaiting_review';
  }
  if (row.status === 'evaluated') {
    return row.passed === true ? 'passed' : 'failed';
  }
  return 'pending';
}
