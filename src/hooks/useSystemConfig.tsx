...
export const EMAIL_NOTIFICATION_LABELS: Record<EmailNotificationKey, { label: string; description: string }> = {
  ...
  interview_scheduled: {
    ...
    description: 'Notify interviewers and panelists instantly when an interview is scheduled.',
  },
  interview_cancelled: {
    label: 'Interview cancelled',
    description: 'Notify interviewers and panelists when a scheduled interview is cancelled.',
  },
  interviewer_daily_digest: {
    label: 'Interviewer daily digest',
    description: 'Email each interviewer and panelist a morning summary of today\'s interviews (9 AM IST).',
  },
  assignment_completed: {
...
export const SUB_PROCESSORS = [
  {
    name: 'Supabase (PostgreSQL, Auth, Storage, Edge Functions)',
    purpose: 'Primary application database, authentication, file storage, serverless functions',
    location: 'Deployer-selected region (self-hosted or cloud)',
  },
  {
    name: 'Amazon Web Services — Simple Email Service (SES)',
    purpose: 'Transactional email delivery (auth, applicant, staff notifications)',
    location: 'Deployer-selected AWS region',
  },
  {
    name: 'Google Gemini',
    purpose: 'Resume parsing, candidate analysis, assessment generation, Chitragupta insights',
    location: 'Google Cloud (cross-border; subject to Google DPA)',
  },
  {
    name: 'Piston (code execution)',
    purpose: 'Sandboxed execution of candidate coding assessment submissions',
    location: 'Self-hosted recommended (see PISTON_URL); public emkc.org fallback is dev-only',
  },
] as const;
...
export function parseComplianceSettings(raw: unknown): ComplianceSettings {
  ...
  return {
    privacy_policy_url: typeof c.privacy_policy_url === 'string' ? c.privacy_policy_url : null,
    terms_url: typeof c.terms_url === 'string' ? c.terms_url : null,
    grievance_officer_name: typeof c.grievance_officer_name === 'string' ? c.grievance_officer_name : null,
    grievance_officer_email: typeof c.grievance_officer_email === 'string' ? c.grievance_officer_email : null,
  };
}

export function parseSsoSettings(raw: unknown): SsoSettings {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
  ...
  return {
    ...
    domain: typeof c.domain === 'string' && c.domain.trim() ? c.domain.trim() : null,
    provider_id: typeof c.provider_id === 'string' && c.provider_id.trim() ? c.provider_id.trim() : null,
  };
}
...
export function useComplianceSettings() {
  const { configValue, isLoading, update, updateAsync, isUpdating } = useSystemConfig('compliance_settings');
  const compliance = parseComplianceSettings(configValue);
  return { compliance, isLoading, update, updateAsync, isUpdating };
}

export function useSsoSettings() {
  const { configValue, isLoading, update, updateAsync, isUpdating } = useSystemConfig('sso_settings');
  const sso = parseSsoSettings(configValue);
  return { sso, isLoading, update, updateAsync, isUpdating };
}
    ...
    queryFn: async () => {
      ...
      if (config_value == null) {
        normalized = configKey === 'cert_tiers' ? {} : configKey === 'business_branding'
          ...
          : configKey === SOCIAL_DRAFTS_ENABLED_KEY
            ? { ...DEFAULT_SOCIAL_DRAFTS_ENABLED }
          : configKey === 'compliance_settings'
            ? { ...DEFAULT_COMPLIANCE_SETTINGS }
          : configKey === 'sso_settings'
