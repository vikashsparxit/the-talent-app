...
import { defaultRatingsForCriteria, mapDraftRatingsToCriteria } from '@/lib/scorecardTemplates';
...
export function InterviewFeedbackDialog({
  ...
  // Reset form when interview or template changes (notes loaded separately — avoid wiping)
  useEffect(() => {
  ...
  // Seed notes from prop + DB when dialog opens (never auto-save this seed)
  useEffect(() => {
    ...
    const propNotes = interview.interview_notes ?? '';
    setNotes(propNotes);

    void (async () => {
      const { data } = await supabase
        .from('candidate_interviews')
        .select('interview_notes')
        .eq('id', interview.id)
        .maybeSingle();
      if (cancelled || notesDirtyRef.current) return;
      const dbNotes = data?.interview_notes ?? '';
      // Prefer richer of prop vs DB (covers flush race + stale list props)
      const next = (propNotes.length >= dbNotes.length ? propNotes : dbNotes) || propNotes || dbNotes;
      skipNotesSaveRef.current = true;
      setNotes(next);
    })();
  ...
  // Sync live notes from parent (e.g. drawer panel) while user hasn't edited in dialog
  useEffect(() => {
    if (!open || notesDirtyRef.current) return;
    if (interview?.interview_notes == null) return;
    skipNotesSaveRef.current = true;
    setNotes(interview.interview_notes);
  }, [open, interview?.interview_notes]);

  // Load draft saved from the candidate drawer notes panel when dialog opens.
  // Wait for scorecard criteria so legacy AI keys (technical) can map onto
  // template keys (technical_depth) before we consume localStorage.
  useEffect(() => {
    if (!open || !interview?.id || criteria.length === 0) return;
    ...
    try {
      ...
      const defaultRatings = defaultRatingsForCriteria(criteria);
      const existing = (interview?.rating_categories || {}) as Record<string, number>;
      const mapped = mapDraftRatingsToCriteria(d, criteria);
      if (d.verdict_suggestion) setVerdict(d.verdict_suggestion as InterviewVerdict);
      setRatings({ ...defaultRatings, ...existing, ...mapped });
      if (typeof d.feedback === 'string' && d.feedback) setFeedback(d.feedback);
      localStorage.removeItem(draftKey);
    } catch { /* ignore malformed draft */ }
  }, [open, interview?.id, criteria.length]);
  ...
  // Auto-save notes 1s after user stops typing (skip seed / non-dirty)
  useEffect(() => {
  ...
  const flushNotesSave = async () => {
    if (!interview?.id || !notesDirtyRef.current) return;
  ...
  const handleDraftFeedback = useCallback(async () => {
    ...
    try {
      ...
      setRatings(prev => ({
        ...prev,
        ...mapDraftRatingsToCriteria(d, criteria),
      }));
      if (typeof d.feedback === 'string' && d.feedback) setFeedback(d.feedback);
    } catch (err: unknown) {
  ...
  const handleSubmit = async () => {
    ...
    onSubmit({
      ...
      feedback,
      artifacts: artifacts.filter((a) => a.type !== 'link' || !!safeExternalUrl(a.url)),
  ...
  const handleAddLink = () => {
    ...
    if (!url) {
      setLinkUrlError('Enter a valid http(s) URL');
      return;
    }
    setArtifacts(prev => [...prev, {
      id: crypto.randomUUID(),
          ...
          <div className="flex items-center justify-between">
            <Label className="text-sm font-medium flex items-center gap-1.5">
              ...
              <span className={cn(
                ...
                {notesSaveStatus === 'idle' && '(auto-saves)'}
              </span>
            </Label>
            {notes.trim() && (
              <button
          ...
          {showLinkForm ? (
            <div className="flex flex-col gap-1.5 p-2.5 border rounded-lg bg-muted/20">
              <Input
                placeholder="https://…"
                value={linkUrl}
                onChange={e => { setLinkUrl(e.target.value); if (linkUrlError) setLinkUrlError(null); }}
                className="h-8 text-xs"
                onKeyDown={e => e.key === 'Enter' && handleAddLink()}
              />
              {linkUrlError && (
                <p className="text-[11px] text-destructive">{linkUrlError}</p>
              )}
              <Input
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() => { setShowLinkForm(false); setLinkUrl(''); setLinkLabel(''); setLinkUrlError(null); }}
                >
                  Cancel
                </Button>
              </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn('max-w-lg max-h-[90dvh] overflow-y-auto', contentClassName)}
