...
import { applyInterviewSchedule, cancelScheduledInterview, type InterviewCancelReason, type ScheduleInterviewData } from '@/lib/interviewPanelists';
...
export function PendingFeedbackGate() {
  ...
  const handleCancelConfirm = async (reason: InterviewCancelReason, note?: string) => {
    if (!currentInterview) return;
    setIsSubmitting(true);
    try {
      await cancelScheduledInterview(currentInterview.id, reason, note);
    ...
    return (
      <>
        ...
        <CancelInterviewDialog
          ...
          onOpenChange={(open) => { if (!open && !isSubmitting) setShowCancelDialog(false); }}
          candidateName={currentInterview.candidate?.name}
          isSubmitting={isSubmitting}
