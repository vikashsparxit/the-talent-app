...
import { Upload, FileText, AlertCircle, AlertTriangle, CheckCircle2, X, Download, ArrowRight, ArrowLeft, Pencil, Briefcase, Building2 } from 'lucide-react';
...
const COLUMN_ALIASES: Record<string, string> = {
  ...
  'notice period': 'notes', notice_period: 'notes', 'current location': 'notes',
...
export function BulkImportDialog({ open, onOpenChange }: BulkImportDialogProps) {
  ...
  const handleImport = async () => {
    ...
    for (const c of valid) {
      ...
      if (error) {
        ...
        successCount++;
        if ((inserted as any)?.id) insertedIds.push((inserted as any).id);
      }
    }
  ...
  const downloadTemplate = () => {
    const rows = [
      'name,email,phone,role_applied,current_role,company,experience_years,skills,notes,linkedin_url',
      '"Rahul Sharma","rahul@example.com","+919876543210","Frontend Developer","Senior Engineer","Acme Corp","5","React;TypeScript;CSS","Strong communicator, open to relocation","https://linkedin.com/in/rahul"',
    ];
