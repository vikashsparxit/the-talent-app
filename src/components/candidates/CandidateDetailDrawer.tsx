...
export function CandidateDetailDrawer({
  ...
  const { data: candidateDetails } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('candidates')
        .select('skills, structured_skills, notes, parse_score, enrichment_score, work_experience')
        .eq('id', candidate!.id)
        .maybeSingle();
  ...
  useEffect(() => {
    ...
    const rawExp = (candidateDetails as { work_experience?: unknown[] } | null | undefined)?.work_experience
    ...
    enrichmentTriggeredFor.current = candidate.id;
    supabase.functions.invoke('enrich-company-websites', {
      body: { candidate_id: candidate.id, ...getDevGeminiKeyBody() },
    }).then(({ data }) => {
      if (data?.work_experience) setEnrichedWorkExp(data.work_experience);
    }).catch(() => {});
  }, [open, candidate?.id, (candidateDetails as { work_experience?: unknown[] } | undefined)?.work_experience]);
  ...
  const flushNotesSave = () => {
    if (!activeInterview?.id || !canEditNotes || !notesDirtyRef.current) return;
    if (notesSaveTimerRef.current) clearTimeout(notesSaveTimerRef.current);
    notesSaveTimerRef.current = null;
    void supabase.from('candidate_interviews')
      .update({ interview_notes: notes } as any)
      .eq('id', activeInterview.id)
      .then(({ error }) => {
        if (error) {
  ...
  const handleDraftNotes = async () => {
    ...
    try {
      ...
      if (data?.draft) {
        ...
        // Auto-open the feedback form pre-filled
  ...
  const redFlags: Array<{ type: string; message: string; severity: 'low' | 'medium' | 'high' }> = useMemo(
    () => filterStaleLinkedInRedFlags(
      Array.isArray((candidate as any)?.red_flags) ? (candidate as any).red_flags : [],
  ...
  const c = candidateDetails ? { ...candidate, ...candidateDetails } : candidate;
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
        ...
        {draftPreview && (
          <div className="space-y-2 border rounded-lg p-3 bg-primary/5 text-xs">
            <p className="font-semibold text-foreground flex items-center gap-1.5">
              ...
              Draft Preview
              <span className="ml-auto text-[10px] text-muted-foreground font-normal">opening Feedback form…</span>
            </p>
        ...
        {canEditNotes && !draftPreview && (
          <p className="text-[10px] text-muted-foreground text-center pt-1">
            Auto-saved · Draft with AI to submit feedback here
          </p>
        )}
      </div>
    </>
  ...
  return (
    ...
    <Sheet open={open} onOpenChange={handleDrawerOpenChange} modal={!hideOverlay}>
      <SheetContent
        ...
        className={cn('flex h-full w-full flex-col p-0', sheetClassName ?? 'sm:max-w-[50%]')}
      >
          {mobilePrepSwitcher}
          <div className="relative flex min-h-0 flex-1 flex-col">
          <ScrollArea className="h-full flex-1">
          <div className={cn('space-y-6 p-6', !isDatabaseContext && (showQuestionKitButton || canEditNotes) && 'pb-20')}>
            {/* Header */}
            ...
            {/* AI Summary */}
            {(() => {
              const aiSummary = (candidate as any).ai_summary as string | null;
      ...
      {/* Feedback dialog — opened automatically when Draft with AI completes */}
