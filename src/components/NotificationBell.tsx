...
import { Bell, CalendarDays, CalendarX, Star, CheckCheck, Eye, AlertTriangle, Megaphone, Rocket, Wrench, Link as LinkIcon } from 'lucide-react';
...
const NOTIF_CONFIG: Record<string, { icon: React.ElementType; bg: string; fg: string }> = {
  ...
  interview_cancelled: { icon: CalendarX,     bg: 'bg-rose-100 dark:bg-rose-900/30',     fg: 'text-rose-600 dark:text-rose-400' },
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3.5">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        {a.link_label && a.link_url && safeHref(a.link_url) && (
          <a
            href={safeHref(a.link_url)!}
            target={safeHref(a.link_url)!.startsWith('http') ? '_blank' : undefined}
...
>(({ unreadCount, className, ...props }, ref) => (
  <button
    ...
    type="button"
    className={cn(
      'relative flex h-10 w-10 md:h-9 md:w-9 shrink-0 items-center justify-center rounded-full bg-muted border-2 border-primary/20 text-muted-foreground hover:border-primary/40 hover:text-foreground transition-colors focus:outline-none',
      className,
    )}
