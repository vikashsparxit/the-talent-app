...
import { jobTypeLabels, experienceLevelLabels, jobStatusLabels, experienceYearsLabels, positionTypeLabels } from '@/types/jobs';
...
export default function Jobs() {
  ...
  const [closeJobDialog, setCloseJobDialog] = useState<{ job: Job; positionsFilled: number } | null>(null);
  ...
  const filteredJobs = jobs.filter(j => {
    const matchesSearch = j.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      (j as any).domain?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      j.location?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = filterStatus === 'all' || j.status === filterStatus;
    const matchesDomain = filterDomain === 'all' || (j as any).domain === filterDomain;
    const matchesTeam = filterTeam === 'all' || j.department === filterTeam;
    const matchesPositionType = filterPositionType === 'all' || (j as any).position_type === filterPositionType;
    const matchesExperience = filterExperience === 'all' || (j as any).experience_years_range === filterExperience;
    return matchesSearch && matchesStatus && matchesDomain && matchesTeam && matchesPositionType && matchesExperience;
  });

  const activeFilterCount = [filterStatus, filterDomain, filterTeam, filterPositionType, filterExperience].filter(f => f !== 'all').length;
  ...
  const handleStatusChange = async (job: Job, newStatus: JobStatus) => {
    if (newStatus === 'closed') {
      // Intercept close → show dialog to capture positions filled
      const selectedCount = (job as any).selected_count ?? 0;
      setCloseJobDialog({ job, positionsFilled: selectedCount });
      return;
    }
            ...
            <Table unwrapped>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className={cn('min-w-[150px]', tableHeadSticky)}>Candidates & Pipeline</TableHead>
                  ...
                  return (
                    <TableRow key={job.id} className="group cursor-pointer" onClick={() => setDetailJob(job)}>
                      ...
                      <TableCell className="text-center">
                        {job.status === 'closed' && (job as any).positions_filled != null
                          ? <span className="font-medium">{(job as any).positions_filled}<span className="text-muted-foreground font-normal">/{(job as any).total_openings || 1}</span></span>
                          : (job as any).total_openings || 1}
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => navigate(`/hiring?view=list&job=${job.id}`)}>
                              ...
                              {canManageJobs && (
                                <DropdownMenuItem onClick={() => setRecruiterDialog({ open: true, jobId: job.id, jobTitle: job.title })}>
                  ...
                  <CardContent className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1 font-medium text-primary" title="Total candidates">
                        ...
                        {(job as any).candidate_count || 0} candidate{(job as any).candidate_count !== 1 ? 's' : ''}
                        {(job as any).total_openings > 1 && (
                          <span className="text-muted-foreground font-normal text-xs">/ {(job as any).total_openings}</span>
                        )}
                      </span>
              ...
              return (
                <>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    <Button onClick={() => { navigate(`/hiring?view=list&job=${detailJob.id}`); setDetailJob(null); }} className="gap-2">
                      <Eye className="h-4 w-4" />
                      View Candidates ({(detailJob as any).candidate_count || 0})
                    </Button>
                    <Button variant="outline" onClick={() => { navigate(`/hiring?view=board&job=${detailJob.id}`); setDetailJob(null); }} className="gap-2">
                    ...
                    <Button variant="outline" onClick={() => copyPublicLink(detailJob.id)} className="gap-2">
                      <Copy className="h-4 w-4" />
                      Copy Link
                    </Button>
                    {canManageJobs && (
                      <Button variant="outline" onClick={() => { setRecruiterDialog({ open: true, jobId: detailJob.id, jobTitle: detailJob.title }); setDetailJob(null); }} className="gap-2">
        ...
        <Dialog open={!!closeJobDialog} onOpenChange={(open) => !open && setCloseJobDialog(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              ...
              <DialogDescription>
                This job has <strong>{(closeJobDialog?.job as any)?.total_openings ?? 1}</strong> opening{((closeJobDialog?.job as any)?.total_openings ?? 1) !== 1 ? 's' : ''}.
                How many candidates actually joined?
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2 py-2">
              ...
              <Input
                ...
                type="number"
                min={0}
                max={(closeJobDialog?.job as any)?.total_openings ?? 99}
                value={closeJobDialog?.positionsFilled ?? 0}
                onChange={(e) => setCloseJobDialog(prev => prev ? { ...prev, positionsFilled: Math.max(0, parseInt(e.target.value) || 0) } : prev)}
              />
            </div>
            <DialogFooter>
