import { useState, useEffect, useMemo, useCallback, type ComponentProps, type CSSProperties, type ReactNode } from 'react';
import {
  DndContext,
  PointerSensor,
  closestCenter,
...
import type { Job, JobStatus, JobType, ExperienceLevel, PositionType, ExperienceYearsRange, JobListTab } from '@/types/jobs';
import {
  jobTypeLabels,
  experienceLevelLabels,
  jobStatusLabels,
  experienceYearsLabels,
  positionTypeLabels,
  isActiveJobStatus,
  isCompletedJobStatus,
  isJobStatusInListTab,
  JOB_CANCEL_REASONS,
} from '@/types/jobs';
...
const statusConfig: Record<JobStatus, { label: string; className: string }> = {
  ...
  cancelled: { label: 'Cancelled', className: 'bg-slate-200 text-slate-700 border-slate-300 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-600' },
};
...
function ClosedJobTooltipWrap({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>This job is no longer active</TooltipContent>
    </Tooltip>
  );
}

function compareJobsBySortOrder(a: Job, b: Job): number {
  return (a.sort_order ?? 0) - (b.sort_order ?? 0) || b.created_at.localeCompare(a.created_at);
}

function JobPriorityMark({ rank }: { rank: number }) {
  return (
    <span
      className={cn(
        'shrink-0 tabular-nums text-[11px] leading-none tracking-tight',
        rank === 1 ? 'font-bold text-primary' : 'font-medium text-muted-foreground',
      )}
      title={`Priority ${rank}`}
    ...
    >
      {canDrag && (
        <TableCell className="w-8 px-2" onClick={(e) => e.stopPropagation()}>
          <button
            type="button"
            className="flex h-8 w-6 items-center justify-center rounded text-muted-foreground cursor-grab active:cursor-grabbing hover:text-foreground"
            aria-label="Reorder job"
            {...attributes}
            {...listeners}
          >
...
function JobLifecycleMenuItems({
  job,
  canManageJobs,
  onStatusChange,
}: {
  ...
  onStatusChange: (job: JobRow, status: JobStatus) => void;
}) {
  if (!canManageJobs) return null;
  return (
    <>
      {job.status === 'draft' && (
        <DropdownMenuItem onClick={() => onStatusChange(job, 'open')}>
      ...
      {isActiveJobStatus(job.status) && (
        <>
          ...
          <DropdownMenuItem onClick={() => onStatusChange(job, 'cancelled')}>
  ...
  if (closed) {
    return (
      <Tooltip>
        ...
        <TooltipContent side="left">This job is no longer active</TooltipContent>
      </Tooltip>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-0.5">
      {hired.map((c) => (
        <Link
          key={c.id}
          to={`/hiring?view=list&profile=${c.id}`}
...
export default function Jobs() {
  ...
  const [cancelJobDialog, setCancelJobDialog] = useState<JobRow | null>(null);
  ...
  const hiredLookupIds = useMemo(
    () => [...closedJobs, ...cancelledJobs].map((j) => j.id),
    [closedJobs, cancelledJobs],
  );
  const { data: hiredByJob = new Map<string, HiredCandidateSummary[]>() } = useJobHiredCandidates(hiredLookupIds);
  ...
  const tabJobs = jobListTab === 'active' ? activeJobs : jobListTab === 'closed' ? closedJobs : cancelledJobs;
  const showHiredColumn = jobListTab === 'closed';

  const priorityByJobId = useMemo(() => {
    const ranked = [...activeJobs].sort(compareJobsBySortOrder);
    return new Map(ranked.map((job, index) => [job.id, index + 1]));
  }, [activeJobs]);
  ...
  const handleStatusChange = async (job: JobRow, newStatus: JobStatus) => {
    ...
    if (newStatus === 'cancelled') {
      setCancelReason('');
      setCancelOtherNote('');
      setCancelJobDialog(job);
      return;
    }
    if (newStatus === 'open' && isCompletedJobStatus(job.status)) {
      await updateJob.mutateAsync({
        id: job.id,
        status: 'open',
        positions_filled: job.status === 'closed' ? 0 : job.positions_filled,
        status_note: null,
      });
      if (detailJob?.id === job.id) {
        setDetailJob({ ...detailJob, status: 'open', positions_filled: job.status === 'closed' ? 0 : job.positions_filled, status_note: null });
      }
      return;
    }
  ...
  const handleConfirmCancel = async () => {
    if (!cancelJobDialog) return;
    const selected = JOB_CANCEL_REASONS.find((r) => r.value === cancelReason);
    let statusNote: string | null = null;
    if (cancelReason === 'other') {
      statusNote = cancelOtherNote.trim() || selected?.label || null;
    } else if (selected) {
      statusNote = selected.label;
    }
    await updateJob.mutateAsync({
      id: cancelJobDialog.id,
      status: 'cancelled',
      status_note: statusNote,
    });
    setCancelJobDialog(null);
    if (detailJob?.id === cancelJobDialog.id) setDetailJob(null);
  };

  const jobDragSensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 6 } }),
  );

  const handleJobDragEnd = useCallback((event: DragEndEvent) => {
    if (!isAdminOrHR || jobListTab !== 'active') return;
    ...
    const orders = filteredJobs.map((j) => j.sort_order ?? 0).sort((a, b) => a - b);
    reorderJobs.mutate(reordered.map((job, index) => ({ id: job.id, sort_order: orders[index] })));
  }, [filteredJobs, isAdminOrHR, jobListTab, reorderJobs]);
  ...
  const canReorder = canManageJobs && jobListTab === 'active' && viewMode === 'list';
      ...
      <Select value={filterStatus} onValueChange={setFilterStatus}>
        ...
        <SelectContent>
          ...
          {Object.entries(jobStatusLabels)
            .filter(([val]) => isJobStatusInListTab(val as JobStatus, jobListTab))
            .map(([val, label]) => (
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 min-w-0 max-w-full">
      <PullToRefresh onRefresh={handleRefresh} className="flex flex-col flex-1 min-h-0">
      <main className="container mx-auto px-4 py-4 md:py-8 pb-safe space-y-4 md:space-y-6 min-w-0 max-w-full">
        <PageHeader
          title="Jobs"
          ...
          actions={
            ...
            <Button
              ...
              asChild
            >
              <Link to="/careers" aria-label="View public careers page">
        ...
        {isLoading ? (
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-muted-foreground mb-4">
                {jobListTab === 'closed'
                  ? 'Jobs closed after a hire appear here.'
                  : jobListTab === 'cancelled'
                    ? 'Cancelled openings (dropped with no hire) appear here.'
          ...
          <Card className="surface-card border-0 overflow-hidden min-w-0 max-w-full">
            <div className="overflow-x-auto min-w-0">
            <DndContext sensors={jobDragSensors} collisionDetection={closestCenter} onDragEnd={handleJobDragEnd}>
            <Table unwrapped>
              <TableHeader>
                <TableRow>
                  {canReorder && <TableHead className={cn('w-8 px-2', tableHeadSticky)}><span className="sr-only">Reorder</span></TableHead>}
                  ...
                  <TableHead className={cn('min-w-[150px]', tableHeadSticky)}>
                    {showHiredColumn ? 'Hired' : 'Candidates & Pipeline'}
                  </TableHead>
              ...
              <TableBody>
                ...
                {filteredJobs.map(job => {
                  ...
                  const isInactive = isCompletedJobStatus(job.status);
                  const hired = hiredByJob.get(job.id) ?? [];
                  const rank = isActiveJobStatus(job.status) ? priorityByJobId.get(job.id) : undefined;
                  return (
                    <SortableJobTableRow
                      key={job.id}
                      id={job.id}
                    ...
                    >
                      <TableCell className="font-medium max-w-[200px]">
                        <div className="flex items-center gap-1.5 min-w-0">
                          {rank != null && <JobPriorityMark rank={rank} />}
                          <Link
                            to={`/hiring?view=board&job=${job.id}`}
                            className="block min-w-0 truncate hover:text-primary hover:underline"
                            onClick={(e) => e.stopPropagation()}
                          >
                            {job.title}
                          </Link>
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-0.5">
                          ...
                          {job.status === 'cancelled' && job.status_note && (
                            <span className="text-[10px] text-muted-foreground leading-none">{job.status_note}</span>
                          )}
                        </div>
                      </TableCell>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 min-w-0">
            {filteredJobs.map(job => {
              ...
              return (
                <Card
                  key={job.id}
                  className="group min-w-0 overflow-hidden surface-card border-0 hover:shadow-elev-2 transition-shadow cursor-pointer"
                  onClick={() => setDetailJob(job)}
                >
                  <CardHeader className="flex flex-row items-start justify-between gap-2 space-y-0 pb-2 min-w-0">
                    <div className="space-y-1 flex-1 min-w-0">
                      <CardTitle className="flex items-center gap-1.5 text-lg min-w-0">
                        {rank != null && <JobPriorityMark rank={rank} />}
                        <Link
                          to={`/hiring?view=board&job=${job.id}`}
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge className={cn("text-xs border", config.className)}>{config.label}</Badge>
                        {job.status === 'cancelled' && job.status_note && (
                          <span className="text-xs text-muted-foreground">{job.status_note}</span>
                        )}
        ...
        <Sheet open={!!detailJob} onOpenChange={(open) => !open && setDetailJob(null)}>
          <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
            {detailJob && (() => {
              ...
              const isInactive = isCompletedJobStatus(detailJob.status);
                ...
                <>
                  <SheetHeader className="pb-4">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      ...
                      {detailJob.status === 'cancelled' && detailJob.status_note && (
                        <span className="text-xs text-muted-foreground">{detailJob.status_note}</span>
                      )}
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    <ClosedJobTooltipWrap closed={isInactive}>
                      <Button
                        disabled={isInactive}
                    ...
                    {canManageJobs && detailJob.status === 'paused' && (
                      <Button variant="outline" onClick={() => { handleStatusChange(detailJob, 'open'); setDetailJob(null); }} className="gap-2">
                        <Play className="h-4 w-4" />
                        Resume
                      </Button>
                    )}
                    {canManageJobs && isActiveJobStatus(detailJob.status) && (
                    ...
                    {canManageJobs && isActiveJobStatus(detailJob.status) && (
                      <Button variant="outline" onClick={() => { handleStatusChange(detailJob, 'cancelled'); setDetailJob(null); }} className="gap-2">
                        <Ban className="h-4 w-4" />
                        Cancel
                      </Button>
                    )}
                    {canManageJobs && isCompletedJobStatus(detailJob.status) && (
                      <Button variant="outline" onClick={() => { handleStatusChange(detailJob, 'open'); setDetailJob(null); }} className="gap-2">
        ...
        {/* Cancel Job Dialog */}
        <Dialog open={!!cancelJobDialog} onOpenChange={(open) => !open && setCancelJobDialog(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>Cancel Job — {cancelJobDialog?.title}</DialogTitle>
              <DialogDescription>
                Not pursuing this opening. No hire. This is not the same as Close (filled/completed).
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-3 py-2">
              ...
              {cancelReason === 'other' && (
                <div className="space-y-2">
                  <Label htmlFor="cancel-other">Note</Label>
                  <Input
                    id="cancel-other"
                    maxLength={120}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setCancelJobDialog(null)}>Back</Button>
              <Button
                variant="destructive"
