...
import { openCandidateDetailWithFetch, fetchVisibleCandidateById } from '@/lib/candidateDetail';
import {
...
import { applyInterviewSchedule, cancelScheduledInterview, formatInterviewScheduleError, insertInterviewSessionsAfterVerdict, moveCandidateToStage, type InterviewCancelReason, type ScheduleInterviewData } from '@/lib/interviewPanelists';
import { CancelInterviewDialog } from '@/components/pipeline/CancelInterviewDialog';
...
export default function Pipeline({ embedded = false }: PipelineProps) {
  ...
  const deepLinkCandidateId = searchParams.get('candidate') || searchParams.get('profile');
  ...
  const [cancelTarget, setCancelTarget] = useState<HolisticInterview | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchVisibleCandidateById(deepLinkCandidateId).then((full) => {
      if (cancelled || !full) return;
      openDrawerCandidate(full);
      clearCandidateParam();
    });
    return () => {
      cancelled = true;
  ...
  const handleCancelInterview = async (reason: InterviewCancelReason, note?: string) => {
    if (!cancelTarget) return;
    setIsCancelling(true);
    try {
      await cancelScheduledInterview(cancelTarget.id, reason, note);
      setCancelTarget(null);
      toast({ title: 'Interview cancelled', description: 'The slot is open again at the same stage.' });
      await queryClient.invalidateQueries({ queryKey: ['candidate-interviews', activeJobId] });
      await queryClient.invalidateQueries({ queryKey: ['scheduled-interviews'] });
      await queryClient.invalidateQueries({ queryKey: ['my-interviews-upcoming'] });
      await queryClient.invalidateQueries({ queryKey: ['pending-feedback-interviews'] });
      await queryClient.invalidateQueries({ queryKey: ['candidate-interview-history'] });
      queryClient.invalidateQueries({ queryKey: ['interview-kit', cancelTarget.id] });
    } catch (err: unknown) {
      toast({ title: 'Error', description: formatInterviewScheduleError(err), variant: 'destructive' });
    } finally {
  ...
  const handleDeclinePending = async (candidateId: string) => {
    ...
    if (!reason) {
      ...
      toast({ title: 'Reason required', description: 'Enter why this candidate is being declined.', variant: 'destructive' });
      return;
    }
  ...
  const pipelineContent = (
    ...
    await queryClient.invalidateQueries({ queryKey: ['pipeline-job-counts'] });
      ...
      <CancelInterviewDialog
        open={!!cancelTarget}
        onOpenChange={(open) => { if (!open && !isCancelling) setCancelTarget(null); }}
        candidateName={cancelTarget?.candidate?.name}
      ...
      <Dialog open={!!declineConfirm} onOpenChange={(open) => { if (!open) { setDeclineConfirm(null); setDeclineReason(''); setDeclineReasonError(false); } }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <div className="space-y-1.5 py-1">
            <label className="text-sm font-medium" htmlFor="pending-decline-reason">
              Reason for declining <span className="text-destructive">*</span>
            </label>
            <textarea
              id="pending-decline-reason"
              className={cn(
                'w-full rounded-md border bg-background px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring placeholder:text-muted-foreground',
                declineReasonError && 'border-destructive focus:ring-destructive',
              )}
              rows={3}
            ...
            />
            {declineReasonError ? (
              <p className="text-xs text-destructive">A reason is required so recruiters can improve screening.</p>
            ) : (
              <p className="text-xs text-muted-foreground">Required — helps recruiters source more relevant profiles.</p>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setDeclineConfirm(null); setDeclineReason(''); setDeclineReasonError(false); }}>Cancel</Button>
            <Button
              variant="destructive"
...
function stageDotClass(stageId: string): string {
  let hash = 0;
  for (let i = 0; i < stageId.length; i++) {
    hash = (hash * 31 + stageId.charCodeAt(i)) | 0;
  }
  return STAGE_DOT_COLORS[Math.abs(hash) % STAGE_DOT_COLORS.length];
}
    ...
    <div ref={setNodeRef} className="w-[320px] shrink-0">
      <div className="flex items-center justify-between mb-3 px-1 group">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {isEditing ? (
            ...
            <>
              <span className={cn('h-2 w-2 rounded-full shrink-0', stageDotClass(stageId))} aria-hidden />
              <h3 className="font-semibold text-sm text-foreground truncate">{stageName}</h3>
              <Badge variant="secondary" className="text-[10px] font-medium rounded-full px-2 py-0 min-w-[1.25rem] justify-center shrink-0 tabular-nums">{count}</Badge>
            </>
          )}
        </div>
        {!isEditing && canManage && (
      ...
      <SortableContext items={interviewIds} strategy={verticalListSortingStrategy}>
        <div className={cn(
          ...
          isOver ? 'bg-primary/5 ring-2 ring-primary/25' : 'bg-muted/50',
        )}>
          {children}
        </div>
      </SortableContext>
