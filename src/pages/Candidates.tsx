...
import { mapParsedResume } from '@/lib/parsedResume';
import { openCandidateDetailWithFetch, fetchVisibleCandidateById, canBypassSharedProfileAccessCheck, candidateInitials } from '@/lib/candidateDetail';
import { openResumeUrl } from '@/lib/resumeStorage';
import { CANDIDATE_SOURCE_SELECT_OPTIONS, candidateSourceLabel } from '@/lib/candidateSources';
import { parseHiringFiltersFromSearch } from '@/lib/searchIntent';
...
function emptyCandidateForm(overrides: Partial<CandidateFormState> = {}): CandidateFormState {
  return {
    name: '',
    email: '',
    phone: '',
...
/** Drop identity/parse fields from a previous CV; keep job, source, notes, referral. */
function resetParseDerivedFields(prev: CandidateFormState): CandidateFormState {
  return emptyCandidateForm({
    job_id: prev.job_id,
    source: prev.source,
    notes: prev.notes,
    referred_by: prev.referred_by,
  });
}
...
export default function Candidates({ mode = 'pipeline', embedded = false }: CandidatesProps) {
  ...
  const { user, isAdmin, isAdminOrHR, isRecruiter, isInterviewer, isSuperAdmin } = useAuth();
  ...
  const handleRefresh = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ['candidate-tags'] });
  };
  ...
  const [advancedOpen, setAdvancedOpen] = useState(() =>
    !!(searchParams.get('skills') || searchParams.get('expMin') || searchParams.get('expMax')
      || searchParams.get('location') || searchParams.get('stage')
      || searchParams.get('name') || searchParams.get('role')),
  );
  ...
  const [advSearch, setAdvSearch] = useState(() => ({
    ...
    expMax: searchParams.get('expMax') ?? '',
  ...
  // Hydrate search/filter state from URL on client-side navigation (⌘K, shared links)
  useEffect(() => {
    const filters = parseHiringFiltersFromSearch(location.search);
    setSearchQuery(filters.q);
    setSkillFilter(filters.skills);
    setAdvSearch(filters.advanced);
    if (filters.hasAdvancedFilters) setAdvancedOpen(true);
  }, [location.search]);
  ...
  const hasNlFilters = skillFilter.length > 0 || !!stageFilter || !!locationFilter
    || !!advSearch.expMin || !!advSearch.expMax;
  ...
  const { candidates, totalCount: serverTotalCount, isLoading, createCandidate, updateCandidate, deleteCandidate, refetch } = useCandidates({
    page: currentPage,
    pageSize,
    // Interviewers on standalone Candidates/Database use talent-db mode; Hiring list stays pipeline.
    mode: isDatabase || (isInterviewer && !embedded) ? 'database' : 'pipeline',
  ...
  const stageFilterIdsKey = stageFilter && forceFullFetch
    ? candidates.map((c) => c.id).join(',')
    : '';

  const { data: stageFilterMap = new Map<string, string>() } = useQuery({
    queryKey: ['candidate-stage-filter', stageFilter, stageFilterIdsKey],
    enabled: !!stageFilter && !!stageFilterIdsKey,
    staleTime: 30_000,
    queryFn: async () => {
      ...
      for (let i = 0; i < ids.length; i += chunkSize) {
        ...
        for (const row of data || []) {
          const stage = row.job_interview_stage as { stage_name?: string; order_index?: number } | null;
          if (!stage?.stage_name || !row.candidate_id) continue;
          const existing = map.get(row.candidate_id);
          const order = stage.order_index ?? 0;
          if (!existing || order > existing.order) {
            map.set(row.candidate_id, { name: stage.stage_name, order });
          }
        }
      }
  ...
  // Only merge fields for the same id — never paint another list row onto the open profile
  // (that preserved ai_summary from candidate A while overwriting identity with B).
  useEffect(() => {
    if (!detailCandidate) return;
    const openId = detailCandidate.id;
    const updated = candidates.find(c => c.id === openId);
    if (!updated) return;
    setDetailCandidate(prev => {
      if (!prev || prev.id !== openId || prev.id !== updated.id) return prev;
      return { ...prev, ...updated, skills: updated.skills ?? prev.skills };
    });
  }, [candidates, detailCandidate?.id]);

  // Auto-open candidate detail drawer from ?profile=<id> shareable link.
  // The hiring list is paginated and pipeline-filtered — absence from `candidates`
  // is not an ACL. Admin / HR / super admin always open; others follow RLS.
  const [profileAccessDenied, setProfileAccessDenied] = useState(false);
  useEffect(() => {
    const profileId = searchParams.get('profile');
    if (!profileId || detailCandidate?.id === profileId) return;

    let cancelled = false;
    const listed = candidates.find(c => c.id === profileId);
    if (listed) {
      void openCandidateDetailWithFetch(listed, setDetailCandidate);
      setProfileAccessDenied(false);
      return;
    }

    const privileged = canBypassSharedProfileAccessCheck({ isAdminOrHR, isSuperAdmin });

    void fetchVisibleCandidateById(profileId).then((full) => {
      if (cancelled) return;
      if (full) {
  ...
  }, [searchParams, candidates, isLoading, detailCandidate, isAdminOrHR, isSuperAdmin]);

  const [form, setForm] = useState<CandidateFormState>(() => emptyCandidateForm());

  const ignoreInFlightParse = () => {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm(prev => resetParseDerivedFields(prev));
    setIsUploading(true);
    try {
      ...
      setIsParsing(true);
      toast({ title: `Parsing ${file.name}…`, description: 'Extracting candidate details with AI' });
      ...
      try {
        ...
        if (parseData?.success && parseData.data) {
          const mapped = mapParsedResume(parseData.data);
          setForm(prev => ({
            ...prev,
            ...
            role_applied: mapped.current_role,
            skills: mapped.skills.join(', '),
            parse_score: mapped.parse_score,
            experience_years: mapped.experience_years,
            candidate_current_role: mapped.current_role,
            candidate_current_company: mapped.current_company,
            work_experience: mapped.work_experience as any[] | undefined,
            education: mapped.education as any[] | undefined,
            certifications: mapped.certifications as any[] | undefined,
            awards: mapped.awards as any[] | undefined,
            credential_score: mapped.credential_score,
            resume_url: fileName,
          }));
  ...
  const filterKey = `${deferredSearch}|${skillFilter.join(',')}|${jobFilter}|${statusFilter}|${stageFilter}|${locationFilter}|${missingFilter.join(',')}|${pipelineFilter}|${tagFilter}|${JSON.stringify(advSearch)}`;
  ...
  const parseBooleanSearch = (query: string, haystack: string): boolean => {
    ...
    if (tokens.length > 1) {
      return tokens.every((token) => h.includes(token.toLowerCase()));
    }
  ...
  const filteredCandidates = candidates.filter(c => {
    ...
    // Advanced field-level filters (also applied from NLP / URL params)
    if (advSearchActive) {
      const adv = advSearch;
    ...
    if (stageFilter) {
      const stageName = (stageFilterMap.get(c.id) ?? '').toLowerCase();
      if (!stageName.includes(stageFilter.toLowerCase())) return false;
    }
  ...
  const handleFormOpenChange = (open: boolean) => {
    ...
    if (!open) {
      ...
      setForm(emptyCandidateForm({ job_id: jobFilter || '' }));
      if (fileInputRef.current) fileInputRef.current.value = '';
    }
  };
  ...
  useStaffHeaderConfig({
    onAddCandidate: !embedded && canManageCandidates ? openCreate : undefined,
  });
  ...
  const candidatesContent = (
    <>
      ...
      <main className={embedded ? 'space-y-3 min-w-0 max-w-full' : 'container mx-auto px-4 sm:px-6 py-4 md:py-8 pb-safe min-w-0 max-w-full'}>
        {!embedded && (
        ...
        <div className={embedded ? 'space-y-2 md:space-y-3' : 'mb-4 md:mb-6 space-y-2 md:space-y-3'}>
        ...
        {isLoading ? (
          ...
          <Card className="border min-w-0 max-w-full overflow-hidden">
            <div className="overflow-x-auto min-w-0 max-md:max-h-[calc(100dvh-12rem)] max-md:overflow-y-auto">
              <Table unwrapped>
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground space-y-0.5 mt-0.5">
                              ...
                              {(candidate as any).linkedin_url && (() => {
                                const linkedInHref = safeExternalUrl((candidate as any).linkedin_url);
                                return linkedInHref ? (
                                <div className="flex items-center gap-1">
                                  <Linkedin className="h-3 w-3" />
                                  <a href={linkedInHref} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline truncate max-w-[180px]">
                                    LinkedIn
                                  </a>
                                </div>
                                ) : null;
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    {isUploading ? (
                      ...
                      <><Loader2 className="h-4 w-4 animate-spin" /> Parsing {parsingFileName || 'resume'}…</>
                    ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {CANDIDATE_SOURCE_SELECT_OPTIONS.map(({ value, label }) => (
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription className="sr-only">
                Full cover letter text submitted with the candidate application.
              </DialogDescription>
            </DialogHeader>
