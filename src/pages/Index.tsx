    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="container mx-auto px-4 sm:px-6 py-4 md:py-6 lg:py-8 space-y-4 md:space-y-6 lg:space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 md:gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {isInterviewer
                ? 'Your assigned candidates and upcoming interviews'
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {isLoading ? (
            [...Array(isInterviewer ? 2 : 4)].map((_, i) => <Skeleton key={i} className="h-24 md:h-28 rounded-xl" />)
          ) : (
            <>
              {!isInterviewer && (
                <MetricCard
              ...
              <MetricCard
                title={isInterviewer ? 'Assigned Candidates' : 'Active Candidates'}
                value={isInterviewer
                  ? (metrics?.totalCandidates ?? 0).toLocaleString()
                  : (metrics?.activeCandidates ?? 0).toLocaleString()}
                icon={Users}
                color="blue"
                subtitle={!isInterviewer ? 'in pipeline' : undefined}
              />
