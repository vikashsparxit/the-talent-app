...
import {
  ...
  ClipboardCheck, HelpCircle, ChevronUp, ChevronDown, X, AlertTriangle,
} from 'lucide-react';
...
import {
  ...
  findOverlappingInterviewIds,
} from '@/lib/interviewConflicts';

/** Tick so relative countdown labels update while the Upcoming tab is open. */
function useCountdownNow(enabled: boolean, intervalMs = 30_000): number {
...
/** Subtle card surface tint when a verdict is set (Past cards with feedback). */
const VERDICT_CARD_TINT: Record<InterviewVerdict, string> = {
  proceeded: 'border-emerald-200/60 bg-emerald-50/50 dark:border-emerald-800/50 dark:bg-emerald-950/30',
  rejected: 'border-rose-200/60 bg-rose-50/50 dark:border-rose-800/50 dark:bg-rose-950/30',
  hold: 'border-amber-200/60 bg-amber-50/50 dark:border-amber-800/50 dark:bg-amber-950/30',
  no_show: 'border-slate-200/60 bg-slate-50/50 dark:border-slate-700/50 dark:bg-slate-900/30',
};
...
/**
 * Relative countdown for upcoming / in-slot interviews.
 * Uses the same 30-min default slot as conflict detection — after start, never stay on "Starting now".
 */
function formatInterviewCountdown(scheduledAt: string, nowMs = Date.now()): string {
  const start = new Date(scheduledAt).getTime();
  if (Number.isNaN(start)) return '';

  const msUntilStart = start - nowMs;
  const windowMs = DEFAULT_INTERVIEW_DURATION_MINUTES * 60_000;

  if (msUntilStart > 0) {
    const totalMins = Math.floor(msUntilStart / 60_000);
  ...
  const msSinceStart = nowMs - start;
  // First ~3 minutes of the slot
  if (msSinceStart < 3 * 60_000) return 'Starting now';
  if (msSinceStart < windowMs) {
    const minsAgo = Math.max(1, Math.floor(msSinceStart / 60_000));
    return minsAgo <= 1 ? 'In progress' : `Started ${minsAgo} min ago`;
  }
  return 'Ended';
}
...
function RatingSummary({ ratings }: { ratings?: RatingCategories | null }) {
  ...
  const entries = Object.entries(ratings).filter(([, v]) => v != null) as [string, number][];
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-2">
      {entries.map(([key, value]) => (
        <div key={key} className="flex items-center gap-2 min-w-0">
          ...
          <div className="h-1.5 w-14 rounded-full bg-muted overflow-hidden shrink-0" aria-hidden>
            <div
              className="h-full rounded-full bg-foreground/65"
              style={{ width: `${Math.min(100, Math.max(0, (value / 5) * 100))}%` }}
            />
          </div>
          <span className="text-[11px] font-semibold tabular-nums text-foreground">{value}/5</span>
        </div>
      ))}
    </div>
  );
...
function InterviewMeta({
  ...
  /** When true, show time only (date is in the day group header). */
  timeOnly?: boolean;
  ...
  const nowMs = useCountdownNow(showCountdown && !!interview.scheduled_at);
    ...
    <>
      <div className="flex items-center gap-2 flex-wrap mb-2">
        ...
        {interview.scheduled_at && (
          timeOnly ? (
            <span className="inline-flex items-center gap-1 text-sm font-semibold tabular-nums tracking-tight text-foreground">
        ...
        {showCountdown && interview.scheduled_at && (
          <span className="text-xs font-medium text-muted-foreground">
            {formatInterviewCountdown(interview.scheduled_at, nowMs)}
          </span>
        )}
        {conflictBadge && (
          <Badge variant="outline" className="gap-1 text-xs bg-amber-50 text-amber-800 border-amber-300 dark:bg-amber-950/40 dark:text-amber-200 dark:border-amber-700">
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        {showMeetingLink && mode === 'video' && safeExternalUrl(interview.meeting_link) && (
          <a
            href={safeExternalUrl(interview.meeting_link)!}
            target="_blank"
...
function UpcomingCard({
  ...
  const mode = interview.interview_mode || 'video';
  const style = MODE_STYLES[mode] || MODE_STYLES.video;
  const meetingHref = safeExternalUrl(interview.meeting_link);
  const canJoin = mode === 'video' && !!meetingHref;
  const verdictTint = interview.verdict ? VERDICT_CARD_TINT[interview.verdict] : null;

  return (
    <div
      className={cn(
        'rounded-xl border p-4 sm:p-5 transition-shadow hover:shadow-md',
        verdictTint ?? style.bg,
        hasConflict
          ? 'border-amber-400 dark:border-amber-600 ring-1 ring-amber-300/60 dark:ring-amber-700/50'
          : !verdictTint && style.border,
      )}
    >
      {hasConflict && (
        <div className="mb-3 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-2.5 py-2 text-xs text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">
          <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5 text-amber-600 dark:text-amber-400" />
          <span>
            Schedule conflict — another interview overlaps this slot. Ask HR/recruiter to reschedule one of them.
          </span>
        </div>
      )}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-stretch sm:justify-between sm:gap-6">
        <div className="min-w-0 flex-1">
          <InterviewMeta
            ...
            conflictBadge={hasConflict}
          />
        </div>
        <div className="flex shrink-0 flex-wrap items-end justify-end gap-2 sm:flex-col sm:items-stretch">
          <Button size="sm" variant="outline" className="gap-1.5" onClick={() => onViewKit(interview)}>
            <HelpCircle className="h-3.5 w-3.5" />
            View question kit
          </Button>
          {canJoin && (
            <Button size="sm" className="gap-1.5 btn-gradient text-primary-foreground" asChild>
...
function PastCard({
  ...
  const verdictTint = interview.verdict ? VERDICT_CARD_TINT[interview.verdict] : null;
  const hasRatings = interview.rating_categories
    && Object.values(interview.rating_categories).some(v => v != null);
  const hasBody = hasRatings || !!interview.feedback || !!interview.interview_notes;

  return (
    <div
      className={cn(
        'rounded-xl border p-4 sm:p-5 transition-shadow hover:shadow-md',
        verdictTint ?? cn(style.bg, style.border),
      )}
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-stretch sm:justify-between sm:gap-6">
        <div className="min-w-0 flex-1">
          ...
          {hasBody && (
            <div className="mt-4 space-y-3 border-t border-border/50 pt-3">
              {hasRatings && <RatingSummary ratings={interview.rating_categories} />}
              ...
              {interview.interview_notes && (
                <div className="flex gap-2 text-xs text-muted-foreground">
                  <NotebookPen className="h-3.5 w-3.5 shrink-0 mt-0.5 opacity-70" />
                  <p className="whitespace-pre-wrap leading-relaxed">{interview.interview_notes}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex shrink-0 flex-wrap items-end justify-end gap-2 sm:flex-col sm:items-stretch sm:justify-between">
          <div className="flex flex-wrap items-center gap-2 sm:flex-col sm:items-end sm:gap-1.5">
            {interview.verdict ? (
              ...
              <Badge
                variant="outline"
                className="text-xs bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400"
              >
                Feedback pending
              </Badge>
            )}
...
type DayGroup = {
  dayKey: string;
  relativeLabel: RelativeDayLabel | null;
...
function groupInterviewsByDay(
  ...
  return Array.from(grouped.entries())
    ...
    .map(([dayKey, dayInterviews]) => {
      ...
      const { relative, absolute } = formatCalendarDayHeaderParts(
...
function DayGroupHeader({
  ...
  return (
    <h2 className="text-sm mb-3">
      <span className="font-semibold text-foreground">{relativeLabel}</span>
      <span className="font-medium text-muted-foreground"> · {absoluteLabel}</span>
    </h2>
  );
}
      ...
      {groups.map(group => (
        <section key={group.dayKey}>
          <DayGroupHeader
            ...
            absoluteLabel={group.absoluteLabel}
          />
          <div className="space-y-3">
            {group.interviews.map(interview => (
              variant === 'upcoming' ? (
                <UpcomingCard
                  ...
                  interview={interview}
                  userTimezone={userTimezone}
                  hasConflict={conflictIds?.has(interview.id)}
                  onViewKit={onViewKit}
                  onOpenCandidateProfile={onOpenCandidateProfile}
                />
              ) : (
...
function InterviewList({
  ...
  const conflictIds = variant === 'upcoming'
    ? findOverlappingInterviewIds(interviews)
    : undefined;

  return (
    <div className="space-y-6">
      {variant === 'upcoming' && conflictIds && conflictIds.size > 0 && (
        <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">
          <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5 text-amber-600 dark:text-amber-400" />
          <p>
            You have {conflictIds.size} interview{conflictIds.size !== 1 ? 's' : ''} with overlapping times.
            Ask the recruiter or HR to reschedule so you are not double-booked.
          </p>
        </div>
      )}
      <DayGroupedList
