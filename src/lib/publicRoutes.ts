...
const ELEVATED_STAFF_ROLES = ['admin', 'hr', 'recruiter'] as const;
...
export function isApplicantUser(
  ...
  if (role && (ELEVATED_STAFF_ROLES as readonly string[]).includes(role)) return false;
  return true;
}

/** Internal staff — no applicant profile, or elevated staff who also have one. */
export function isStaffUser(
  role: string | null | undefined,
  hasApplicantProfile: boolean,
): boolean {
  if (hasApplicantProfile) {
    return !!(role && (ELEVATED_STAFF_ROLES as readonly string[]).includes(role));
  }
