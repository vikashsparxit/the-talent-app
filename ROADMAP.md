...
### Agency / multi-client ATS
**Status:** Planned — not started · **Priority:** Later (founder take-up)

Employer ATS (hire for yourself) vs recruitment agency (hire for many client companies). Different from subdomain SaaS above: one agency deployment, many clients — not one TTA customer per subdomain. **Not in the active build queue.** SparxIT internal agency ATS plan (private repo only; not linked here).
...
- Strips legal suffixes and city names ("Pvt Ltd Noida" → clean brand name), then Tavily searches the cleaned name
- Gemini picks the official homepage from those hits (hallucinated domains discarded); results stored in `work_experience` JSONB and cached in `external_refs` (`provider = tavily_company`, 30-day TTL)
